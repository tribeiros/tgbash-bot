...
app.post('/new-message', function(req, res) {
  ...
  const botTelegram = '673240626:AAH-F00rEr6t2E72GjG2Gm29M0qaH-SNU8E' //tribeirosanycodeBot
...
function checkCommand(param) {
  ...
  if (botCommands[0] === 'touch' && botCommands[1] === undefined) {
    checkedBash = 'missing file argument'
    return checkedBash
  }
  ...
  switch (botCommands[0]){
    ...
    case 'touch':
      checkedBash = shell.touch(botCommands[1]).stdout
      return checkedBash
  }
...
//console.log(command);
  ...
  } else if (command === "" && botCommands.includes('touch')){
    command = `succeeded touch ${botCommands[1]}`
  }
